...
Use the burned-subtitles version as the publishing default. `Chinese_playful_streamer_nv1` at `1.10` speed is the active voice configuration in `video/episode-04/generate_minimax_narration.mjs`. Preserve the native pitch and volume by omitting the `pitch` and `vol` parameters; do not set speed below `1.00`.
